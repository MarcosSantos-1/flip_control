...
export default function CNCPage() {
  ...
  const urgentCount = useMemo(
    () => cncs.filter((cnc) => cnc.status?.toLowerCase() === "urgente").length,
    [cncs]
  );
  ...
  const loadCNCs = async () => {
    try {
      ...
      console.error("Erro ao carregar CNCs:", error);
      setCncs([]);
      setTotal(0);
    } finally {
      setLoading(false);
      ...
      <div className="space-y-8">
        <div className="relative overflow-hidden rounded-xl bg-linear-to-r from-orange-600/10 via-orange-600/5 to-transparent p-8 border border-orange-200/50 dark:border-orange-800/50">
          ...
          <div className="relative">
            <h1 className="text-4xl font-bold tracking-tight bg-linear-to-r from-orange-600 to-amber-500 bg-clip-text text-transparent pb-2">CNCs</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="hover:shadow-md transition-all duration-200 border-l-4 border-l-orange-500">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Registros de CNC dentro do período selecionado
              </p>
            </CardContent>
          </Card>
          <Card className="hover:shadow-md transition-all duration-200 border-l-4 border-l-amber-500">
            ...
            <CardContent>
              <div className="text-4xl font-bold text-amber-600 dark:text-amber-400">{cncs.length}</div>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-muted-foreground flex flex-col items-center gap-2">
            ...
            <p className="text-lg font-medium text-foreground">
              Nenhuma CNC encontrada para {monthLabel}.
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
            {cncs.map((cnc) => {
              const isUrgente = cnc.status?.toLowerCase() === "urgente";
              return (
                <Card
                  key={cnc.id}
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        <div className="flex flex-wrap items-center gap-3">
                          <h3 className="text-lg font-bold font-mono text-primary">BFS: {cnc.bfs}</h3>
                          {isUrgente && (
                        ...
                        <div className="flex items-start gap-2 text-sm text-muted-foreground">
                          ...
                          <span>{cnc.endereco || "Endereço não informado"}</span>
                        </div>

                        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground pt-2">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-foreground">Subprefeitura:</span> {cnc.subprefeitura}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-foreground">Prazo:</span> {cnc.prazo_hours}h
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-foreground">Abertura:</span> {format(new Date(cnc.data_abertura), "dd/MM/yyyy HH:mm")}
                          </div>
                        </div>
                      </div>
                    </div>
